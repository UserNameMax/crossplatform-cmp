...
/**
 * Обёртка вокруг списка — ровно то, что студенты увидят уже на ПЗ1 19.09: класс
 * состояния MVI в шаблоне устроен так же. Поле нестабильного типа делает
 * нестабильным весь класс.
 *
 * Разница не в самом классе, а в том, кто его создаёт: пришедший из ViewModel
 * экземпляр живёт дольше кадра и пропуск не ломает, пересозданный в теле
 * composable — ломает на каждой рекомпозиции.
 */
